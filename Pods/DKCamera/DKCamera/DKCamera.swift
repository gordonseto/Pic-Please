...
public class DKCamera: UIViewController {
    ...
    public func handleFocus(gesture: UITapGestureRecognizer) {
        ...
        } else if let currentDevice = self.currentDevice where currentDevice.exposurePointOfInterestSupported {
            let touchPoint = gesture.locationInView(self.view)
            self.exposureAtTouchPoint(touchPoint)
        }
    }
    
    private func exposureAtTouchPoint(touchPoint: CGPoint) { ////////////////////////////////////////////
